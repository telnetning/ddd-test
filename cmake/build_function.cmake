...
function(install_timezone)
    ...
    if("${with_system_tzdata}" STREQUAL "")
        separate_arguments(ZIC_COMMAND UNIX_COMMAND "COMMAND ./zic -d ${PROJECT_SRC_DIR}/share/xxx/timezone -p 'US/Eastern' ${TZDATA_FILES}")
    else()
        separate_arguments(ZIC_COMMAND UNIX_COMMAND "COMMAND zic -d ${PROJECT_SRC_DIR}/share/xxx/timezone -p 'US/Eastern' ${TZDATA_FILES}")
    endif()
...
# The chaotic use of a compiler is a dangerous behavior, but Roach temporarily does not allow modification of the suffix name.
...
function(add_cmd_gen_when_configure _target_name src_list)
    ...
    foreach(id RANGE 0 ${src_length} 1)
        ...
        execute_process(
            COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --runscript ${PROJECT_TRUNK_DIR} ${CMAKE_BINARY_DIR} "${before_cmd}" "${main_cmd}" "${thread_cmd}" "${after_cmd}"
...
function(GET_VERSIONSTR_FROMGIT ret)
    ...
    # xxx_VERSION can be accuired from file during compiling, but we can provide some default values
    set(opendstore_VERSION "2.1.0")
    # get opendstore version from ${PROJECT_OPENGS_DIR}
    execute_process(
        COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --get_raw_versionstr ${PROJECT_OPENGS_DIR} OUTPUT_VARIABLE opendstore_VERSION)
    execute_process(
        COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --s ${PROJECT_TRUNK_DIR} OUTPUT_VARIABLE GS_VERSION_STR)
...
function(GET_TIMESTR_FOR_ROACH)
    ...
    execute_process(
        COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --get_time_for_roach ${PROJECT_TRUNK_DIR} OUTPUT_VARIABLE COMPILETIME_STR)
...
function(SET_GCC_FLAGS _flags _defs)
    if($ENV{CONFIG_CROSS})
        set(CMAKE_TOOLCHAIN_FILE "${LOCAL_LIB_PATH}/cmake_tool_chain.cfg" PARENT_SCOPE)
    endif()
    ...
    link_directories(${LOCAL_LIB_PATH}/lib .)
...
MACRO(FIND_PROJECT_LINK_LIBS _type)
    execute_process(COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --find-libs $ENV{TO_3RD}/lib/ "${alib}.${_type}" ${PROJECT_TRUNK_DIR} ${CMAKE_BINARY_DIR} $ENV{DESTDIR}${CMAKE_INSTALL_PREFIX} ERROR_VARIABLE _errno RESULT_VARIABLE _errno_ret OUTPUT_VARIABLE _file_names)
    if("${_file_names}" STREQUAL "")
        execute_process(COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --find-libs ${FOUND_GCC_LIB_PATH} "${alib}.${_type}" ${PROJECT_TRUNK_DIR}  ${CMAKE_BINARY_DIR} $ENV{DESTDIR}${CMAKE_INSTALL_PREFIX} ERROR_VARIABLE _errno RESULT_VARIABLE _errno_ret OUTPUT_VARIABLE _file_names)
    endif()
...
function(PREPARE_FOR_LOGICAL_DECODE)
    execute_process(
        COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --get_logical_decoding_version "${CMAKE_SOURCE_DIR}/${opendstore}" "largeVer" OUTPUT_VARIABLE LARGE_VERSION_STR)
    SET(LGC_DCD_LARGE_VERSION "${LARGE_VERSION_STR}" PARENT_SCOPE)
    execute_process(
        COMMAND ${CMAKE_SOURCE_DIR}/${opendstore}/cmake/buildfunction.sh --get_logical_decoding_version "${CMAKE_SOURCE_DIR}/${opendstore}" "smallVer" OUTPUT_VARIABLE SMALL_VERSION_STR)
    SET(LGC_DCD_SMALL_VERSION "${SMALL_VERSION_STR}" PARENT_SCOPE)
endfunction(PREPARE_FOR_LOGICAL_DECODE)

# Traverse a direcotry and find all its subdirectories
