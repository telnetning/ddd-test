 ...
 *
 * IDENTIFICATION
 *    src/common/algorithm/dstore_crc32c_choose.cpp
 *
 * -------------------------------------------------------------------------
 */
...
#include "dstore_crc32c_choose.h"
...
constexpr uint32 OFFSET_24 = 24;
constexpr uint32 OFFSET_16 = 16;
constexpr uint32 OFFSET_8 = 8;
inline void CalculCrc8(uint32 *crc, uint32 x)
{
#ifdef WORDS_BIGENDIAN
    *crc = CRC32C_TABLE[0][((*crc >> OFFSET_24) ^ (x)) & 0xFF] ^ (*crc << OFFSET_8);
#else
    *crc = CRC32C_TABLE[0][(*crc ^ (x)) & 0xFF] ^ (*crc >> OFFSET_8);
#endif
}

constexpr uint32 FIRST_3_BYTES = 3;
constexpr uint32 CRC_ROW_1 = 0;
constexpr uint32 CRC_ROW_2 = 1;
constexpr uint32 CRC_ROW_3 = 2;
constexpr uint32 CRC_ROW_4 = 3;
constexpr uint32 CRC_ROW_5 = 4;
constexpr uint32 CRC_ROW_6 = 5;
constexpr uint32 CRC_ROW_7 = 6;
constexpr uint32 CRC_ROW_8 = 7;


constexpr uint32 STEP_8 = 8;
constexpr uint32 STEP_4 = 4;
#if defined(__aarch64__)
static uint32 CompCrc32cNeon(uint32 crc, const uint8* data, size_t len)
{
    const uint8* p = data;
    ...
    while (p + STEP_8 <= pend) {
        crc = static_cast<uint32>(__crc32cd(crc, *(static_cast<const uint64*>(static_cast<const void*>(p)))));
        p += STEP_8;
    }

    /* Process remaining full four bytes if any */
    if (p + STEP_4 <= pend) {
        crc = __crc32cw(crc, *(static_cast<const unsigned int*>(static_cast<const void*>(p))));
        p += STEP_4;
    }
...
static uint32 CompCrc32cSb8(uint32 crc, const uint8* data, size_t len)
{
    const uint8* p = data;
    /*
     * Process eight bytes of data at a time.
     */
    const uint32* p4 = static_cast<const uint32*>(static_cast<const void*>(p));
    while (len >= OFFSET_8) {
        uint32 a = *p4++ ^ crc;
        uint32 b = *p4++;
...
#ifdef WORDS_BIGENDIAN
        const uint8 c0 = b;
        const uint8 c1 = b >> OFFSET_8;
        const uint8 c2 = b >> OFFSET_16;
        const uint8 c3 = b >> OFFSET_24;
        const uint8 c4 = a;
        const uint8 c5 = a >> OFFSET_8;
        const uint8 c6 = a >> OFFSET_16;
        const uint8 c7 = a >> OFFSET_24;
#else
        const uint8 c0 = b >> OFFSET_24;
        const uint8 c1 = b >> OFFSET_16;
        const uint8 c2 = b >> OFFSET_8;
        const uint8 c3 = b;
        const uint8 c4 = a >> OFFSET_24;
        const uint8 c5 = a >> OFFSET_16;
        const uint8 c6 = a >> OFFSET_8;
        const uint8 c7 = a;
#endif

        crc = CRC32C_TABLE[CRC_ROW_1][c0] ^ CRC32C_TABLE[CRC_ROW_2][c1] ^ CRC32C_TABLE[CRC_ROW_3][c2] ^
              CRC32C_TABLE[CRC_ROW_4][c3] ^ CRC32C_TABLE[CRC_ROW_5][c4] ^ CRC32C_TABLE[CRC_ROW_6][c5] ^
              CRC32C_TABLE[CRC_ROW_7][c6] ^ CRC32C_TABLE[CRC_ROW_8][c7];
     ...
     */
    p = static_cast<const uint8*>(static_cast<const void*>(p4));
    while (len > 0) {
        CalculCrc8(&crc, *p);
        p++;
        len--;
...
static uint32 CompCrc32cSse42(uint32 crc, const uint8* data, size_t len)
{
    const uint8* p = data;
    const unsigned char* pend = p + len;
    ...
    while (p + STEP_8 <= pend) {
        crc = static_cast<uint32>(_mm_crc32_u64(crc, *(static_cast<const uint64*>(static_cast<const void*>(p)))));
        p += STEP_8;
    }

    /* Process remaining full four bytes if any */
    if (p + STEP_4 <= pend) {
        crc = _mm_crc32_u32(crc, *(static_cast<const unsigned int*>(static_cast<const void*>(p))));
        p += STEP_4;
    }
...
constexpr uint32 EXX_1 = 0;
constexpr uint32 EXX_2 = 1;
constexpr uint32 EXX_3 = 2;
constexpr uint32 EXX_4 = 3;
constexpr uint32 SSE_4_2 = 20;
static bool Crc32cSse42Available(void)
{
    unsigned int exx[4] = {0, 0, 0, 0};
    (void)__get_cpuid(1, &exx[EXX_1], &exx[EXX_2], &exx[EXX_3], &exx[EXX_4]);
    return (exx[EXX_3] & (1 << SSE_4_2)) != 0; /* SSE 4.2 */
}

#endif

static uint32 CompCrc32cChoose(uint32 crc, const uint8* data, size_t len);
uint32 (*g_compCrc32c)(uint32 crc, const uint8* data, size_t len) = CompCrc32cChoose;
/*
...
{
#if defined(__aarch64__)
    g_compCrc32c = CompCrc32cNeon;
#else
    if (Crc32cSse42Available()) {
        g_compCrc32c = CompCrc32cSse42;
    } else {
        g_compCrc32c = CompCrc32cSb8;
    }
#endif

    return g_compCrc32c(crc, data, len);
}
