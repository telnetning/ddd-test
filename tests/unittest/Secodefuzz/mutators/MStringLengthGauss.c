...
static int StringLengthGauss_getcount(S_Element* pElement)
{
...
static char* StringLengthGauss_getvalue(S_Element* pElement, int pos)
{
    int i;
    int out_len;
    int in_len;
...
static int StringLengthGauss_getissupport(S_Element* pElement)
{
    ASSERT_NULL(pElement);
    //有初始值的字符串
...
const struct Mutater_group StringLengthGauss_group = {
    .name = "StringLengthGauss",
    .getCount = StringLengthGauss_getcount,
    .getValue = StringLengthGauss_getvalue,
    .getIsSupport = StringLengthGauss_getissupport,
};

void init_StringLengthGauss(void)
{
    register_Mutater(&StringLengthGauss_group, enum_StringLengthGauss);
}
