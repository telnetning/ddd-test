...
int BlobChangeToNull_getcount(S_Element* pElement)
{
...
char* BlobChangeToNull_getvalue(S_Element* pElement, int pos)
{
    int i;
    int in_len;
...
int BlobChangeToNull_getissupport(S_Element* pElement)
{
    //只要是字符串就支持
    ASSERT_NULL(pElement);
...
const struct Mutater_group BlobChangeToNull_group = {
    "BlobChangeToNull", BlobChangeToNull_getcount, BlobChangeToNull_getvalue, BlobChangeToNull_getissupport, 1};

void init_BlobChangeToNull(void)
{
    register_Mutater(&BlobChangeToNull_group, enum_BlobChangeToNull);
}
